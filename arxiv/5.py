...
async def crawl_website(url, retry_count=0):
    try:
        async with async_playwright() as p:
            ...
            titles_elements = await page.query_selector_all('p.title ')
            papers = []
            for i, paper_element in enumerate(paper_elements):
                ...
                if i < len(abstract_elements):
                    ...
                    paper_authors = paper_authors[9:]
                    # change the string to list
                    paper_authors = paper_authors.split(',')
                    title = await titles_elements[i].inner_text()
                else:
                    abstract_text = ''
        ...
        if retry_count < MAX_RETRIES:
            ...
            return await crawl_website(url, retry_count + 1)
        else:
...
async def main():
    ...
    start_indexes = list(range(0, 20000, 25))  # Adjust the range as needed
    urls = [base_url + f'&start={start_index}' for start_index in start_indexes]

    async with asyncio.Semaphore(MAX_WORKERS):
        tasks = []
        for url in urls:
            task = asyncio.create_task(crawl_website(url))
            tasks.append(task)

        all_papers = await asyncio.gather(*tasks)

    # Flatten the list of papers
    all_papers = [paper for page_papers in all_papers for paper in page_papers]

    # Dump the papers data into a JSON file
    with open('badcase-full-4-p.json', 'w') as file:
