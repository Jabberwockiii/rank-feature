...
async def main():
    ...
    async def scrape_page_with_semaphore(start_index):
        async with semaphore:
            return await scrape_page(start_index)

    tasks = []
    for start_index in range(0, max_index, step):
        task = asyncio.create_task(scrape_page_with_semaphore(start_index))
        tasks.append(task)
    ...
    with open('badcase-full-2-p.json', 'w') as file:
        json.dump(all_papers, file, indent=2)

    print(f'Scraped {len(all_papers)} papers and saved to badcase-full-2-s.json')
